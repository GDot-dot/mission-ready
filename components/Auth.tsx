    ...
    try {
      if (isRegistering) {
        if (!identifier.includes('@')) {
          setError('新帳號請使用 Email 註冊');
          setIsLoading(false);
          return;
        }

        const result = await cloudAuth.register(identifier, password, displayName);
        if (result.success && result.userId) {
          ...
          if (!storedUsers.some((u: any) => u.id === result.userId)) {
             localStorage.setItem('mission_ready_users', JSON.stringify([...storedUsers, {
               id: result.userId,
               username: result.username || displayName || identifier,
               email: result.email || identifier,
               authProvider: 'firebase'
             }]));
          }
          onLogin({ id: result.userId, username: result.username || displayName || identifier, email: result.email || identifier, authProvider: 'firebase' });
        } else {
          setError(result.error || '註冊失敗');
        }
      } else {
        const result = await cloudAuth.login(identifier, password);
        if (result.success && result.userId) {
          localStorage.setItem('mission_ready_active_user_id', result.userId);
          const storedUsers = JSON.parse(localStorage.getItem('mission_ready_users') || '[]');
          const nextUser = {
            ...
            email: result.email,
            authProvider: result.authProvider || (identifier.includes('@') ? 'firebase' : 'legacy')
          };
          localStorage.setItem('mission_ready_users', JSON.stringify([
            ...storedUsers.filter((u: any) => u.id !== result.userId),
            nextUser
          ]));
          onLogin(nextUser);
        } else {
          // 如果雲端失敗，嘗試本地登入 (作為備援)
          const storedUsers = JSON.parse(localStorage.getItem('mission_ready_users') || '[]');
          const localUser = storedUsers.find((u: any) => u.username === identifier && u.password === password);
          if (localUser) {
             console.log('Cloud login failed, using local credentials');
             onLogin({ id: localUser.id, username: localUser.username, authProvider: 'legacy' });
          } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-100 dark:bg-slate-900 p-4 transition-colors duration-200">
      <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl w-full max-w-md border border-slate-200 dark:border-slate-700">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{isRegistering ? 'Email' : 'Email 或舊帳號'}</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder={isRegistering ? 'name@example.com' : '輸入 Email 或舊帳號'}
              />
            </div>
          </div>
